...
def evaluate_dialogue(dialogue, additional_knowledge, human_human_dialogue, evaluators, api_url, first_speaker, secondary_speaker, try_number=1):
    while True:
        ...
        # average +1 std aggregation
        # if score_list[0] >= 0.9775896369963996 and score_list[1] >= 0.83273268353539886 and score_list[2] >= 0.8559167212756334:
        #     print(f'evaluation end. The confidence scores are {score_list}')
        #     return (dialogue,
        #             try_number,
        #             count_speaker_occurrences(human_human_dialogue, first_speaker),
...
if __name__ == "__main__":
    ...
    csv_file_path = '../sampled_HHDs/O_Middle_DT_example_dialogues.csv'

    output_csv_path = 'average_-0.5_std_aggregation_output_adapted_dialogues/O_Middle_DT_HCD.csv'
    ...
    with open(output_csv_path, mode='a', newline='', encoding='utf-8') as output_file:
        ...
        for human_human_dialogue in human_human_dialogues:
            ...
            **Important!!!** Your final output format must strictly follow the format of the provided Human-Human Dialogue, and you must directly output the transformed new dialogue without including any additional title or text in your response!!!
            ...
            final_dialogue, try_number, hhd_fs_count, hhd_ss_count, hcd_fs_count, hcd_ss_count = evaluate_dialogue(human_computer_dialogue, additional_knowledge, human_human_dialogue, evaluators,
                                       api_url, first_speaker, secondary_speaker)

            # Write the row with the index
            writer.writerow({'Index': index, 'Dialogue': final_dialogue, 'Try_Number': try_number, 'hhd_fs_count': hhd_fs_count, 'hhd_ss_count': hhd_ss_count, 'hcd_fs_count': hcd_fs_count, 'hcd_ss_count': hcd_ss_count})
